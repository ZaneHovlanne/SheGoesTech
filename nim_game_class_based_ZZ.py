...
class ComputerPlayer:
    ...
    def get_move(self, match_count, min_remove, max_remove):
        ...
        if match_count == 9 or match_count == 5:
            take = 3
        elif match_count == 8 or match_count == 4:
            take = 2
        elif match_count == 7 or match_count == 6 or match_count == 3:
            take = 1
        else:
            take = random.randint(min_remove, max_remove)
        return take
...
class HumanPlayer:
    ...
    def get_move(self, match_count, min_remove, max_remove):
        ...
        while True:
            try:
                removed_matches = int(
                    input("How many matches do you want to remove? "))
...
class NimGame:
    ...
    def get_player_input(self):
        if self.is_player_a_turn:
            ...
            # we need to pass some data to the player
            return self.player_a.get_move(self.match_count, self.min_matches, self.max_matches)
        else:
            # we need to pass some data to the player
            return self.player_b.get_move(self.match_count, self.min_matches, self.max_matches)
   ...
   # we will use a method to update the state
    ...
    def print_winner(self):
        if self.is_player_a_turn:
            # a because we already switched the turn
            print(f"{self.player_a.name} wins!")
...
if __name__ == "__main__":
    ...
    player_a = HumanPlayer("Valdis")
    # Google made AlphaGo and AlphaZero for chess and Go
    player_b = ComputerPlayer("Alpha Nim")

    # using default values
    game = NimGame(player_a=player_a, player_b=player_b)
    game.play()
